...
function human.checkIfBulletNear()
    local emotion = "Happy"
    local x,y = bullet.headPosition()
    local searchStartX, searchStartY = x - 1, y - 1
    local searchDStartX, searchDStartY = x - 2, y - 2
    -- Anger
    for idx, hum in ipairs(humans) do
        for i = 0, 4 do
            for j = 0, 4 do
                if searchDStartX + i == hum.x and searchDStartY+j == hum.y then
                    emotion = "Dissappointed"
                end      
            end
        end    
        for i = 0, 2 do
            for j = 0, 2 do
                if searchStartX + i == hum.x and searchStartY+j == hum.y then
                    emotion = "Angry"
                end      
            end
        end    
